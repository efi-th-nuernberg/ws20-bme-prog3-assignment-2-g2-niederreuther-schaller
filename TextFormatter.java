    ...
    /* Alle Wörter von aText werden der ArrayList "words" hinzugefügt*/
		...
		for(int count =0; count < words.size(); count++)
      {
		    if (bText.length() + words.get(count).length() < 30) //solang bisheriger Text in der Zeile und zu prüfendes Wort kleiner sind als 30 Zeichen
        {
          ...
          bringRight(bText);// oder (falls 30 ereicht sind) geben wir den Text aus
          bText = words.get(count) + " ";
        }
		}
    bringRight(bText); //für letzte Zeile, welche keine 30 Zeichen mehr beinhaltet:
    //restlicher bText wird ausgegeben
	}

  private void bringRight(String formText){ //ermittelt Differenz zwischen Zeilenlänge und Stringlänge
    int diff= 30-formText.length();
          int temp=0;
          while (temp!=diff) //solange der Zähler temp ungleich der Differenz, wird ein Leerzeichen an den Start angefügt
          {
            formText= " "+formText;
            temp++;
          }
